...
while Running:
    ...
    if Debug:
        ...
        if round((clock.get_fps())) < 60:
            fpstext = font.render(f"Fps: {round(clock.get_fps())}", True, (255 - round((clock.get_fps()) * 4.25), round((clock.get_fps()) * 4.25), 0))
        else:
            fpstext = font.render(f"Fps: {round(clock.get_fps())}", True, (0, 255, 0))
